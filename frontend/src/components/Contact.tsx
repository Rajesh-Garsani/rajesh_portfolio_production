      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-7 bg-[#121212] border border-neutral-800 rounded-3xl p-6 md:p-8">
          <AnimatePresence mode="wait">
            ...
            </motion.form> : <motion.div key="success" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="min-h-[360px] flex flex-col items-center justify-center text-center gap-5"><div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center"><CheckCircle className="w-8 h-8 text-emerald-400"/></div><div><h3 className="text-lg font-extrabold text-white">Message Received</h3><p className="text-sm text-neutral-400 mt-2 max-w-sm">Your inquiry has been securely stored and a notification will be sent when email delivery is configured.</p></div><button type="button" onClick={() => setIsSubmitted(false)} className="text-xs text-emerald-400 font-mono hover:text-emerald-300">Send another message</button></motion.div>}
          </AnimatePresence>
        </div>
      </div>
    </section>
